...
def run_query(query):
    # ✅ Sanitize malformed SQL before executing
...
def data_comprehension(question, context):
    # Limit number of rows sent to the model to 3-5 items
    limited_context = context[:5]  # Keep only top 5 items
    ...
    chat_completion = client_sql.chat.completions.create(
        messages=[
            ...
            {
                "role": "user",
                "content": f"QUESTION: {question}. DATA: {limited_context}",
            }
        ],
        model=os.environ['GROQ_MODEL'],
        temperature=0.2,
